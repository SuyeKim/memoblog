...
public class MemoService {
    ...
    public MemoResponseDto getPost(Long id) {
        Memo memo = memoRepository.findById(id).orElseThrow(//memoRepository DB에서 가져옴
    ...
    public MemoResponseDto updateMemo(Long id, MemoRequestDto requestDto, HttpServletRequest request) {
        ...
        if (token != null) {
            ...
            if (jwtUtil.validateToken(token)) {
                ...
                memo = memoRepository.findById(id).orElseThrow(//memoRepository (memo Entity)DB에서 가져옴.
                        () -> new IllegalArgumentException("아이디가 존재하지 않습니다.")
                );
